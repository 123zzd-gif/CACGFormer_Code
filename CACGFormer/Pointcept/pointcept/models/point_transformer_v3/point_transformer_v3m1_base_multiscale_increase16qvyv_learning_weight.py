...
"""
Enhanced Point Transformer - V3 Model

Based on the original work by Xiaoyang Wu (xiaoyang.wu.cs@gmail.com)
Enhanced for better small region segmentation
"""
...
class ClassBalancedAttention(PointModule):
    def __init__(self, channels, num_classes, sparse_indices=range(8, 16), 
        ...
        # 打印一下初始化信息，便于调试
        print(f"初始化类别平衡注意力模块: 输入通道={channels}, 类别数={num_classes}, "
        ...
        if learnable_weights:
            ...
            # 可选：添加权重约束，确保权重为正
            self.weight_activation = nn.Softplus()  # 确保权重为正
    ...
    def _build_attention_network(self, channels):
        """构建注意力网络，便于重新构建"""
        hidden_dim = max(channels // 4, 64)  # 至少保持64个通道
    ...
    def forward(self, point):
        ...
        if feat_dim != self.channels:
            print(f"自动调整: 特征维度 {feat_dim} 与预期通道数 {self.channels} 不匹配")
            # 重新创建注意力层以匹配输入维度
            device = point.feat.device
            self.channels = feat_dim  # 更新通道数信息
        ...
        # 计算每个点属于不同类别的注意力权重
        attn_logits = self.attention(point.feat)  # [N, num_classes]
        attn_weights = torch.softmax(attn_logits, dim=1)  # [N, num_classes]
...
class SimpleGlobalEncoder(PointModule):
    #"""简化版的全局特征编码器，无论是训练、验证还是测试阶段都使用相同的处理流程"""
...
class PointTransformerV3Enhanced(PointModule):
    def __init__(
        ...
        use_region_aware_pooling=True,  # Changed to False since we're removing RegionAwarePooling
        ...
        self.use_region_aware_pooling = True  # Always set to False since we're removing RegionAwarePooling
        # 明确指定稀疏类别索引(8-15)
        ...
        if not self.cls_mode:
            ...
            #添加 SimpleGlobalEncoder 实例化
            # if use_region_aware_pooling:
            #     self.simple_global_encoder = SimpleGlobalEncoder(
            #         in_channels=enc_channels[0] + sum(dec_channels[:-1]),  # 32 + (64 + 64 + 128 + 256) = 544
            #         out_channels=enc_channels[0] + sum(dec_channels[:-1])  # 544
            #     )
    def forward(self, data_dict):
        ...
        embed_feat = point.feat  # 保存嵌入层输出特征
        point = self.enc(point)

        
        if not self.cls_mode:
            dec_features = [embed_feat]  # 第一个特征改为嵌入层输出
            pooling_inverses = [None]  # 嵌入层没有pooling_inverse
            ...
            for i in range(len(self.dec)):
                ...
                if self.use_class_balanced_attn:
                    ...
                    if 0 <= rev_idx < len(self.class_attn_modules):
                        point = self.class_attn_modules[rev_idx](point)
