...
g_classes = ['yayin', 'yaguan']  # 只有两个类别
g_class2label = {cls: i for i, cls in enumerate(g_classes)}  # 将类别映射为标签
g_class2color = {
    'yayin': [255, 0, 0],  # yayin 类别使用红色
    'yaguan': [0, 255, 0]   # yaguan 类别使用绿色
}
...
def collect_point_label(anno_path, out_filename, file_format='txt'):
    ...
    Args:
        ...
        file_format: 'txt' or 'npy', determines the file format to save.
    Returns:
        None
    """
    points_list = []
    for f in glob.glob(os.path.join(anno_path, '*.txt')):  # Process all txt files
        print(f)
        points = np.loadtxt(f)  # Load point cloud data file, which contains XYZ, normals, and labels
        xyz = points[:, :3]  # Get XYZ coordinates
        labels = points[:, 6].reshape(-1, 1)  # Get labels (assuming labels are the last column)

        # Set color to white (RGB: 255, 255, 255)
        rgb = np.ones((points.shape[0], 3)) * 255  # Color set to white

        # Combine XYZ coordinates, RGB color, and labels
        data = np.concatenate([xyz, rgb, labels], axis=1)  # Nx6, containing XYZ, color, and label
        points_list.append(data)
    
    data_label = np.concatenate(points_list, 0)  # Concatenate all data from different files

    # Shift the points so the minimum coordinates are at the origin
    xyz_min = np.amin(data_label, axis=0)[:3]
    data_label[:, :3] -= xyz_min

    # Save the data based on the specified file format
    if file_format == 'txt':
        np.savetxt(out_filename, data_label, fmt='%f %f %f %d %d %d %d')  # Save as txt
    elif file_format == 'npy':
        np.save(out_filename, data_label)  # Save as numpy .npy file
    else:
