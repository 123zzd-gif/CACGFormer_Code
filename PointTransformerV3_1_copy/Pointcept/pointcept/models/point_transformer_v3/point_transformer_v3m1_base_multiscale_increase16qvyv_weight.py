...
class ClassBalancedAttention(PointModule):
    def __init__(self, channels, num_classes, sparse_indices=range(8, 16), balance_weights=None):
        super().__init__()
        self.channels = channels
        ...
        # 打印一下初始化信息，便于调试
        print(f"初始化类别平衡注意力模块: 输入通道={channels}, 类别数={num_classes}, 稀疏类别={self.sparse_indices}")
        ...
        if balance_weights is None:
            ...
            for idx in self.sparse_indices:
                self.balance_weights[idx] = 3.0  # 为稀疏类别设置更高权重
        else:
    ...
    def forward(self, point):
        ...
        # 根据类别权重对注意力进行调整
        weighted_attn = attn_weights * self.balance_weights.to(attn_weights.device)
        #weighted_attn = weighted_attn / weighted_attn.sum(dim=1, keepdim=True)###验证一下这个地方
