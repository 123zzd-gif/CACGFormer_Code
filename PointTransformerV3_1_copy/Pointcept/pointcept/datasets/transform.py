...
class Collect(object):
    ...
    def __call__(self, data_dict):
        ...
        for name, keys in self.kwargs.items():
            ...
            #     data[name] = torch.cat([data_dict[keys].float()], dim=1)
            # else:
            ...
            if len(keys) == 1:
                ...
                for k in keys:
                    if k in data_dict:
                        tensors.append(data_dict[k].float())
                # 截断到最小长度，保证维度一致
                if len(tensors) > 1:
...
class ChromaticAutoContrast(object):
    ...
    def __call__(self, data_dict):
        if "color" in data_dict.keys() and np.random.rand() < self.p:
            ...
            scale = 255 / (hi - lo + 1e-8)
            contrast_feat = (data_dict["color"][:, :3] - lo) * scale
            contrast_feat = np.nan_to_num(contrast_feat, nan=0.0, posinf=1.0, neginf=-1.0) 
            blend_factor = (
...
class GridSample(object):
    ...
    def __call__(self, data_dict):
        ...
        if self.mode == "train":  # train mode
            ...
            for key in self.keys:
                if key in data_dict: #data_dict[key] = data_dict[key][idx_unique]
...
class Compose(object):
    ...
    def __call__(self, data_dict):
        for t in self.transforms:
            #print(f"Running transform: {t.__class__.__name__}, data_dict type: {type(data_dict)}")###
